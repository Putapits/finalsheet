...
try {
  switch ($action) {
    case 'update': {
      ...
      if (empty($fields))
        jerr('No fields to update');
      $params[':uid'] = $_SESSION['user_id'];
      $sql = 'UPDATE service_requests SET ' . implode(', ', $fields) . ', updated_at = CURRENT_TIMESTAMP WHERE id = :id AND assigned_inspector_id = :uid AND deleted_at IS NULL';
      $stmt = $db->prepare($sql);
    ...
    case 'create': {
      ...
      if (!in_array($service_type, $allowedTypes, true))
        jerr('Invalid service type');
      $user_id = (int) ($input['user_id'] ?? 0);
      $user_email = trim((string) ($input['user_email'] ?? ''));
      if ($user_id <= 0) {
        if ($user_email === '' || !filter_var($user_email, FILTER_VALIDATE_EMAIL))
          jerr('Provide valid user_email or user_id');
        ...
        if (!$row) {
          ...
          if (!$newId)
            jerr('Failed to create citizen account');
          $phone = trim((string) ($input['phone'] ?? ''));
          $address = trim((string) ($input['address'] ?? ''));
          if ($phone !== '' || $address !== '') {
            $database->updateUser((int) $newId, array_filter(['phone' => $phone, 'address' => $address], fn($v) => $v !== ''));
          }
          $user_id = (int) $newId;
        } else {
          $user_id = (int) $row['id'];
      ...
      if ($full_name === '' || $email === '' || !filter_var($email, FILTER_VALIDATE_EMAIL) || $phone === '' || $address === '' || $service_details === '')
